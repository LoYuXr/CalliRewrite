...
def vgg_net(x, n_classes, img_size, reuse, is_train=True, dropout_rate=0.5):
    ...
    with tf.compat.v1.variable_scope('VGG16', reuse=reuse):
        ...
        if is_train:
            x = tf.compat.v1.layers.dropout(x, dropout_rate)

        x = tf.compat.v1.layers.dense(x, 4096, activation=tf.nn.relu)
        if is_train:
            x = tf.compat.v1.layers.dropout(x, dropout_rate)

        out = tf.compat.v1.layers.dense(x, n_classes)
        print(out)
...
def vgg_net_slim(x, img_size):
    return_map = {}
    # Define a scope for reusing the variables
    with tf.compat.v1.variable_scope('VGG16', reuse=tf.compat.v1.AUTO_REUSE):
        x = tf.reshape(x, [-1, img_size, img_size, 1])
